...
instr(cmd) := [i | some i in input; lower(i.Cmd) == cmd]
...
stage_aliases contains alias if {
	...
	count(f.Value) >= 3
	upper(f.Value[count(f.Value)-2]) == "AS"

	alias := f.Value[count(f.Value)-1]
}
...
deny contains msg if {
	...
	msg := sprintf(
		"FROM %q: pin the base image to an explicit non-latest tag (or @sha256 digest)",
		[img],
	)
}
...
deny contains msg if {
	...
	msg := sprintf(
		"use COPY or a verified download instead of ADD %q",
		[v],
	)
}
...
warn contains msg if {
	...
	msg := sprintf(
		"FROM %q is pinned by tag, not digest — pin by @sha256:... for reproducible builds",
		[img],
	)
}
...
warn contains msg if {
	...
	msg := sprintf(
		"ENV token %q may bake a secret into the image — inject secrets at runtime",
		[kv],
	)
}